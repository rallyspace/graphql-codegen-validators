  ...
  initialEmit(): string {
    ...
    return `\n${this.enumDeclarations.join('\n')}\n${
      new DeclarationBlock({})
        .asKind('function')
        .withName(
          'union<T extends {}>(...schemas: ReadonlyArray<yup.Schema<T>>): yup.MixedSchema<T>'
        )
        .withBlock(
          [
        ...
        if (this.config.enumsAsTypes) {
          ...
          this.enumDeclarations.push(
            new DeclarationBlock({})
              ...
              .withName(`${enumname}Schema`)
              .withContent(
                `yup.string<${enumTypeName}>().oneOf(Object.values(${enumTypeName})).defined()`
              ).string
          );
        }
      }
        ...
        switch (this.config.validationSchemaExportType) {
          case 'const':
            return new DeclarationBlock({})
              ...
              .withContent(`union<${unionTypeName}>(${unionElements})`).string;
          case 'function':
          default:
            return new DeclarationBlock({})
              .export()
              .asKind('function')
              .withName(`${unionName}Schema(): yup.MixedSchema<${unionTypeName}>`)
              .withBlock(indent(`return union<${unionTypeName}>(${unionElements})`)).string;
        }
      }
    };
  }
...
function shapeFields(
  ...
  return fields
    ?.map((field) => {
      ...
      if (field.kind === Kind.INPUT_VALUE_DEFINITION) {
        ...
        if (defaultValue?.kind === Kind.STRING || defaultValue?.kind === Kind.ENUM) {
          if (config.useEnumTypeAsDefaultValue && defaultValue?.kind !== Kind.STRING) {
            let value = convertNameParts(
              defaultValue.value,
              resolveExternalModuleAndFn('change-case-all#pascalCase'),
              config?.namingConvention?.transformUnderscore
            );

            if (config.namingConvention?.enumValues)
              value = convertNameParts(
                defaultValue.value,
                resolveExternalModuleAndFn(config.namingConvention?.enumValues),
                config?.namingConvention?.transformUnderscore
              );
      ...
      if (isNonNullType(field.type)) return fieldSchema;
...
function generateFieldYupSchema(
  config: ValidationSchemaPluginConfig,
  visitor: Visitor,
...
function generateFieldTypeYupSchema(
  config: ValidationSchemaPluginConfig,
  visitor: Visitor,
  type: TypeNode,
  parentType?: TypeNode
  ...
  if (isNamedType(type)) {
    ...
    if (isNonNullType(parentType)) {
      if (visitor.shouldEmitAsNotAllowEmptyString(type.name.value)) return `${gen}.required()`;

      return `${gen}.nonNullable()`;
    }
    const typ = visitor.getType(type.name.value);
    if (typ?.astNode?.kind === 'InputObjectTypeDefinition') return `${gen}`;
...
function generateNameNodeYupSchema(
  config: ValidationSchemaPluginConfig,
  visitor: Visitor,
  node: NameNode
): string {
...
function yup4Scalar(
  ...
  if (config.scalarSchemas?.[scalarName]) return `${config.scalarSchemas[scalarName]}.defined()`;
