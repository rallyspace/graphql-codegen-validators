  ...
  initialEmit(): string {
    return `\n${[
      new DeclarationBlock({})
        ...
        .withContent(
          ['Required<{', '  [K in keyof T]: z.ZodType<T[K], any, T[K]>;', '}>'].join('\n')
        ).string,
      // Unfortunately, zod doesn’t provide non-null defined any schema.
      // This is a temporary hack until it is fixed.
      // see: https://github.com/colinhacks/zod/issues/884
      new DeclarationBlock({}).asKind('type').withName('definedNonNullAny').withContent('{}')
        .string,
      new DeclarationBlock({})
        .export()
        .asKind('const')
      ...
      leave: InterfaceTypeDefinitionBuilder(
        ...
        (node: InterfaceTypeDefinitionNode) => {
          ...
          const shape = node.fields
            ?.map((field) => generateFieldZodSchema(this.config, visitor, field, 2))
            .join(',\n');
        ...
        this.enumDeclarations.push(
          this.config.enumsAsTypes
            ...
            : new DeclarationBlock({})
                ...
                .asKind('const')
                .withName(`${enumname}Schema`)
                .withContent(`z.nativeEnum(${enumTypeName})`).string
        );
      }
    };
  }
...
function generateFieldZodSchema(
...
function generateFieldTypeZodSchema(
  config: ValidationSchemaPluginConfig,
  visitor: Visitor,
  field: InputValueDefinitionNode | FieldDefinitionNode,
  type: TypeNode,
...
function generateNameNodeZodSchema(
  config: ValidationSchemaPluginConfig,
  visitor: Visitor,
  node: NameNode
): string {
...
function maybeLazy(type: TypeNode, schema: string): string {
  if (isNamedType(type) && isInput(type.name.value)) return `z.lazy(() => ${schema})`;
